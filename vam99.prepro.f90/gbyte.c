...
#endif
/* Tools for storage/retrieval of arbitrary size bytes from 32/64 bit words
    (note - this version is not currently (6/30/88) described in the
    gbytes document)
 Modified by Arne Jorgensen 12-April-1991 (Sun)
...
# define MASK ULONG_MAX                        /* Mask of sword bits */
# define G1BYTE(p,q,b) ((b==BITS_PER_WORD ? MASK : ~(MASK<<b)) & (p>>(SWORD-(q+b))))
                                               /* Get 1 word contained byte */
# define MASK1(q,b) (b==BITS_PER_WORD ? MASK : (~(MASK<<b)<<(SWORD-(q+b))))
                                               /* Mask of sword bits */
...
#if (defined rs6000) || (defined __hpux)
gbytes(p,u,q,b,s,n)
#else
gbytes_(p,u,q,b,s,n)
#endif
